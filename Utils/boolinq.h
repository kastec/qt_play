...
class Linq {
  ...
  public:
    ...
    QPair<QList<T>,QList<T>>  split(std::function<bool(T)> filter) //const
    {
        QList<T> trueItems;
        QList<T> falseItems;
        
        for_each([&trueItems,&falseItems,filter](T value) {
            auto res = filter(value);
            if(res)
                trueItems.append(value);
            else
                falseItems.append(value);
        });
        ...
        try {
            for (int i = 0; ; i++) {
                auto value = linq.next();
                auto res = filter(value);
                qDebug()<< "i:" << i << res;
                if(res) trueItems.append(value);
                else falseItems.append(value);
            }
        }
        catch (LinqEndException &) { }
*/
        QPair<QList<T>,QList<T>> res;
        res.first = trueItems;
        res.second = falseItems;
